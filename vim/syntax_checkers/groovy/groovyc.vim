"============================================================================
"File:        groovyc.vim
"Description: Groovy syntax checking plugin for syntastic.vim
"             Almost entirely cargo-culted from syntastic javac.vim by:
"             * Jochen Keil <jochen.keil at gmail dot com>
...
let g:syntastic_groovy_groovyc_maven_pom_tags = ['build', 'properties']
let g:syntastic_groovy_groovyc_maven_pom_properties = {}
let s:has_maven = 0
...
if !exists('g:syntastic_groovy_maven_executable')
    let g:syntastic_groovy_maven_executable = 'mvn'
endif
...
if !exists('g:syntastic_groovy_maven_options')
    let g:syntastic_groovy_maven_options = ''
endif
...
if !exists('g:syntastic_groovy_groovyc_delete_output')
    let g:syntastic_groovy_groovyc_delete_output = 1
endif

if !exists('g:syntastic_groovy_groovyc_autoload_maven_classpath')
    let g:syntastic_groovy_groovyc_autoload_maven_classpath = 1
endif

if !exists('g:syntastic_groovy_groovyc_config_file_enabled')
    let g:syntastic_groovy_groovyc_config_file_enabled = 0
endif
...
if !exists('g:syntastic_groovy_groovyc_custom_classpath_command')
    let g:syntastic_groovy_groovyc_custom_classpath_command = ''
endif

if !exists('g:syntastic_groovy_groovyc_maven_pom_ftime')
    let g:syntastic_groovy_groovyc_maven_pom_ftime = {}
endif

if !exists('g:syntastic_groovy_groovyc_maven_pom_classpath')
    let g:syntastic_groovy_groovyc_maven_pom_classpath = {}
endif
...
command! SyntasticgroovycEditClasspath call s:EditClasspath()
command! SyntasticgroovycEditConfig    call s:EditConfig()

" }}}1

function! SyntaxCheckers_groovy_groovyc_IsAvailable() dict " {{{1
    let s:has_maven = executable(expand(g:syntastic_groovy_maven_executable, 1))
    return executable(expand(g:syntastic_groovy_groovyc_executable, 1))
endfunction " }}}1

function! SyntaxCheckers_groovy_groovyc_GetLocList() dict " {{{1
    let groovyc_opts = g:syntastic_groovy_groovyc_options

    let output_dir = ''
    if g:syntastic_groovy_groovyc_delete_output
    ...
    " add classpathes to groovyc_classpath {{{2
    let groovyc_classpath = ''

    for path in split(g:syntastic_groovy_groovyc_classpath, s:ClassSep())
        if path !=# ''
            try
            ...
            if type(ps) == type([])
                for p in ps
                    let groovyc_classpath = s:AddToClasspath(groovyc_classpath, p)
                endfor
            else
                let groovyc_classpath = s:AddToClasspath(groovyc_classpath, ps)
            endif
        endif
    endfor

    if s:has_maven && g:syntastic_groovy_groovyc_autoload_maven_classpath
        if !g:syntastic_groovy_groovyc_delete_output
            let groovyc_opts .= ' -d ' . syntastic#util#shescape(s:MavenOutputDirectory())
        endif
        let groovyc_classpath = s:AddToClasspath(groovyc_classpath, s:GetMavenClasspath())
    endif
    " }}}2

    " load custom classpath {{{2
    if g:syntastic_groovy_groovyc_custom_classpath_command !=# ''
        " Pre-process the classpath command string a little.
        let classpath_command = g:syntastic_groovy_groovyc_custom_classpath_command
        ...
        for l in split(lines, "\n")
            let groovyc_classpath = s:AddToClasspath(groovyc_classpath, l)
        endfor
    endif

    if groovyc_classpath !=# ''
        let groovyc_opts .= ' -cp ' . syntastic#util#shexpand(groovyc_classpath)
    endif
    " }}}2
    ...
    " unashamedly stolen from *errorformat-groovyc* (quickfix.txt) and modified to include error types
    let errorformat =
        \ '%E%f: %l: %m'
...
function! s:SaveClasspath() " {{{2
    ...
    if g:syntastic_groovy_groovyc_config_file_enabled
        if filereadable(expand(g:syntastic_groovy_groovyc_config_file, 1))
            ...
            let lines = readfile(expand(g:syntastic_groovy_groovyc_config_file, 1))
            " strip g:syntastic_groovy_groovyc_classpath options from config file lines
            let i = 0
            while i < len(lines)
                if match(lines[i], 'g:syntastic_groovy_groovyc_classpath') != -1
                    call remove(lines, i)
                else
                    let i += 1
                endif
        ...
        " add new g:syntastic_groovy_groovyc_classpath option to config
        call add(lines, 'let g:syntastic_groovy_groovyc_classpath = ' . string(path))
        " save config file lines
        call writefile(lines, expand(g:syntastic_groovy_groovyc_config_file, 1))
    endif
    " set new classpath
    let g:syntastic_groovy_groovyc_classpath = path
    let &modified = 0
endfunction " }}}2

function! s:EditClasspath() " {{{2
    let command = 'syntastic groovyc classpath'
    let winnr = bufwinnr('^' . command . '$')
    if winnr < 0
        let path = []
        let pathlines = split(g:syntastic_groovy_groovyc_classpath, "\n")
        for p in pathlines
...
function! s:EditConfig() " {{{2
    ...
    let command = 'syntastic groovyc config'
    let winnr = bufwinnr('^' . command . '$')
    if winnr < 0
        let lines = []
        if filereadable(expand(g:syntastic_groovy_groovyc_config_file, 1))
...
function! s:GetMavenProperties() " {{{2
    ...
    if s:has_maven && filereadable(pom)
        if !has_key(g:syntastic_groovy_groovyc_maven_pom_properties, pom)
            let mvn_cmd = syntastic#util#shexpand(g:syntastic_groovy_maven_executable) .
                \ ' -f ' . syntastic#util#shescape(pom) .
                \ ' ' . g:syntastic_groovy_maven_options
            ...
            for line in mvn_settings_output
                ...
                if mvn_is_managed_tag && !empty(matches)
                    let mvn_is_managed_tag = index(g:syntastic_groovy_groovyc_maven_pom_tags, matches[1]) >= 0
            ...
            let g:syntastic_groovy_groovyc_maven_pom_properties[pom] = mvn_properties
        endif
        return g:syntastic_groovy_groovyc_maven_pom_properties[pom]
    endif
...
function! s:GetMavenClasspath() " {{{2
    let pom = syntastic#util#findFileInParent('pom.xml', expand('%:p:h', 1))
    if s:has_maven && filereadable(pom)
        if !has_key(g:syntastic_groovy_groovyc_maven_pom_ftime, pom) || g:syntastic_groovy_groovyc_maven_pom_ftime[pom] != getftime(pom)
            let mvn_cmd = syntastic#util#shexpand(g:syntastic_groovy_maven_executable) .
                \ ' -f ' . syntastic#util#shescape(pom) .
                \ ' ' . g:syntastic_groovy_maven_options
            ...
            let g:syntastic_groovy_groovyc_maven_pom_ftime[pom] = getftime(pom)
            let g:syntastic_groovy_groovyc_maven_pom_classpath[pom] = mvn_classpath
        endif
        return g:syntastic_groovy_groovyc_maven_pom_classpath[pom]
    endif
    return ''
endfunction " }}}2
...
function! s:MavenOutputDirectory() " {{{2
    ...
    if s:has_maven && filereadable(pom)
        ...
        let sep = syntastic#util#Slash()
        let src_main_dir = get(mvn_properties, 'project.build.sourceDirectory', join(['src', 'main', 'groovy'], sep))
        let src_test_dir = get(mvn_properties, 'project.build.testsourceDirectory', join(['src', 'test', 'groovy'], sep))
